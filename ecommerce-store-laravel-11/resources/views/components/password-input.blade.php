<div class="flex flex-col relative" x-data="{show_password:false, toggle(){this.show_password=!this.show_password}}">
    <input id="password" x-bind:type="show_password ? 'text' : 'password'" wire:model="password" class="px-2 py-1 rounded border-2">

    <button x-on:click="toggle" type="button" class="rounded bg-black/10 h-6 w-7 absolute top-1 right-2 flex items-center justify-center">
        <svg x-show="!show_password" class="size-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
